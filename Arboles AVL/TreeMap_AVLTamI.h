...
//
//  Modificación para el problema 13.
//
...
protected:
   ...
   struct TreeNode {
      ...
      TreeNode(clave_valor const& e, Link i = nullptr, Link d = nullptr,
               int alt = 1, int tamI = 1) : cv(e), iz(i), dr(d), altura(alt), tam_i(tamI) {}
   };
...
public:
   ...
   int k_esimo(int const& pos) const {
	   Link p = busca_pos(pos, raiz);
	   if (p == nullptr)
		   return -1;
	   return p->cv.clave;
   }
...
protected:
   ...
   Link busca_pos(int const& c, Link a) const {
	   if (a == nullptr) {
		   return nullptr;
	   }
	   else if (menor(c, a->tam_i)) {
		   return busca_pos(c, a->iz);
	   }
	   else if (menor(a->tam_i, c)) {
		   return busca_pos(c - a->tam_i, a->dr);
	   }
	   else { // c == a->cv.clave
		   return a;
	   }
